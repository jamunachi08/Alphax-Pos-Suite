...
async function load_settings(){
  ...
  async function add_credit_note(){
    ...
    const d=new frappe.ui.Dialog({
      ...
      primary_action: async (v)=>{
        ...
        const r=await frappe.call({method:'alphax_pos_suite.pos.redemption.get_credit_note_available', args:{credit_note: cn}});
  ...
  async function boot_from_terminal(){
    ...
    if(!terminal) return;
    try{
        const r = await frappe.call({method:'alphax_pos_suite.api.get_pos_boot', args:{terminal}});
